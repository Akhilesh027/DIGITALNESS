/**
 * seed_production_final.js
 * Master Production Seeder for Digitalness CRM
 */
...
async function seedProductionFinal() {
  console.log("==================================================================");
  console.log("DIGITALNESS CRM: MASTER PRODUCTION DATA SEEDER");
  console.log("==================================================================\n");
  ...
  await Branch.create({
    ...
    state: "Telangana",
    location: "Gachibowli",
    address: "Gachibowli Main Road, Financial District, Hyderabad",
    contactPhone: "+91 9900112233",
    contactEmail: "admin@digitalness.agency",
    status: "Active",
  });
  console.log("  ✓ Seeded Headquarters Branch (BR001).\n");
  ...
  const campaignsWithIds = campaignsData.map((c) => ({
    _id: new mongoose.Types.ObjectId(),
    ...c,
    createdAt: new Date(),
    updatedAt: new Date(),
  }));
  await AdCampaign.collection.insertMany(campaignsWithIds);
  const campaigns = campaignsWithIds;
  ...
  const periodFormatted = `${currentYear}-${monthStr}`;
  ...
  for (const client of customers) {
    const calendar = await ContentCalendar.create({
      ...
      items: [
        {
          itemKey: `ITEM-${client._id}-01`,
          plannedDate: new Date(now.getTime() + 86400000 * 1),
          contentType: "REEL",
          sourceType: "SERVICE",
          headline: "Behind the Scenes Craftsmanship",
          caption: `Excellence is in every detail at #${client.name.replace(/\s+/g, "")}.`,
          creativeBrief: "High-energy aesthetic video showing expert precision.",
          platformTargets: ["Instagram", "Facebook"],
          status: "APPROVED",
          hashtags: ["#Digitalness", "#AgencyGrowth", "#PremiumExperience", `#${client.businessType.split(" ")[0]}`],
        },
        {
          itemKey: `ITEM-${client._id}-02`,
          plannedDate: new Date(now.getTime() + 86400000 * 3),
          contentType: "CAROUSEL",
          sourceType: "INDUSTRY",
          headline: "Top 5 Industry Insights You Must Know",
          caption: "Swipe through to discover the latest strategies. 👉",
          creativeBrief: "10-slide educational carousel with data points.",
          platformTargets: ["Instagram", "LinkedIn"],
          status: "APPROVED",
          hashtags: ["#Digitalness", "#AgencyGrowth", "#Tips"],
        },
        {
          itemKey: `ITEM-${client._id}-03`,
          plannedDate: new Date(now.getTime() + 86400000 * 5),
          contentType: "SOCIAL_POST",
          sourceType: "EVERGREEN",
          headline: "Founder Perspective on Quality & Scale",
          caption: "Scaling without compromising on bespoke quality.",
          creativeBrief: "Thought leadership text with aesthetic photo.",
          platformTargets: ["LinkedIn"],
          status: "SCHEDULED",
          hashtags: ["#Leadership", "#Scale", "#Quality"],
        },
        {
          itemKey: `ITEM-${client._id}-04`,
          plannedDate: new Date(now.getTime() - 86400000 * 2),
          contentType: "SOCIAL_POST",
          sourceType: "SERVICE",
          headline: "Client Spotlight & Real Results",
          caption: "Hear what our clients have to say!",
          creativeBrief: "Customer quote card layout with high-contrast text.",
          platformTargets: ["Facebook", "Instagram"],
          status: "PUBLISHED",
          hashtags: ["#Testimonial", "#ClientResults"],
        },
      ],
      summary: {
        totalItems: 4,
    ...
    const sampleContentItems = [
      {
        ...
        caption: `Excellence is in every detail at #${client.name.replace(/\s+/g, "")}.`,
        scheduledFor: new Date(now.getTime() + 86400000 * 1),
        status: "Approved",
      ...
      {
        customerId: client._id,
        title: "Top 5 Industry Insights You Must Know",
        contentType: "Carousel",
        platforms: ["Instagram", "LinkedIn"],
        caption: "Swipe through to discover the latest strategies. 👉",
        scheduledFor: new Date(now.getTime() + 86400000 * 3),
        status: "Approved",
        approvalStatus: "Approved",
        publishStatus: "Scheduled",
        createdBy: designer._id,
      ...
      {
        customerId: client._id,
        title: "Founder Perspective on Quality & Scale",
        contentType: "Post",
        platforms: ["LinkedIn"],
        caption: "Scaling without compromising on bespoke quality.",
        scheduledFor: new Date(now.getTime() + 86400000 * 5),
        status: "Scheduled",
        approvalStatus: "Approved",
        publishStatus: "Scheduled",
        createdBy: designer._id,
      ...
      {
        customerId: client._id,
        title: "Client Spotlight & Real Results",
        contentType: "Post",
        platforms: ["Facebook", "Instagram"],
        caption: "Hear what our clients have to say!",
        scheduledFor: new Date(now.getTime() - 86400000 * 2),
        status: "Published",
        approvalStatus: "Approved",
    ...
    await ContentItem.insertMany(sampleContentItems);
  }
  ...
  await Ticket.insertMany(ticketsData.map((t) => ({ ...t, branchId: "BR001", createdBy: adminUser._id })));
  ...
  await SLAIncident.deleteMany({});
  const slaData = [
    {
      incidentKey: "SLA-INC-001",
      workId: createdTasks[0]._id,
      ...
      primaryRootCause: "Revision feedback arrived after hours.",
      resolvedAt: new Date(now.getTime() - 86400000 * 1),
    },
    {
      incidentKey: "SLA-INC-002",
      workId: createdTasks[7]._id,
      clientId: prestigeClient._id,
      type: "SLA_BREACH",
      severity: "LOW",
      riskScore: 40,
      status: "RESOLVED",
      responsibility: "INTERNAL",
      primaryRootCause: "4K ProRes cloud export delay.",
      resolvedAt: new Date(now.getTime() - 86400000 * 3),
    },
    {
      incidentKey: "SLA-INC-003",
      workId: createdTasks[14]._id,
      clientId: cloudScaleClient._id,
      type: "STALLED_REVIEW",
      severity: "LOW",
      riskScore: 35,
      status: "OPEN",
      responsibility: "CLIENT",
      primaryRootCause: "Awaiting final sign-off from CTO.",
    },
  ];
  await SLAIncident.insertMany(slaData.map((s) => ({ ...s, assignedTo: accountManager._id })));
  console.log(`  ✓ Created ${slaData.length} SLA Incidents.\n`);

  // 11. MARKETING CONNECTIONS
  console.log(">>> [11/12] Seeding Marketing Connections...");
  await MarketingConnection.deleteMany({});
  const connectionsData = [
    { customerId: auraClient._id, platform: "Meta", accountType: "FacebookPage", platformAccountId: "act_meta_aura_77218", platformAccountName: "Aura Aesthetics Clinic Ad Account", status: "Connected", accessToken: "EAAG_simulated_token_aura_aesthetics", connectedAt: new Date(now.getTime() - 86400000 * 30), connectedBy: adminUser._id },
    { customerId: prestigeClient._id, platform: "Meta", accountType: "FacebookPage", platformAccountId: "act_meta_prestige_99312", platformAccountName: "Prestige Living Master Ad Account", status: "Connected", accessToken: "EAAG_simulated_token_prestige_skyvillas", connectedAt: new Date(now.getTime() - 86400000 * 45), connectedBy: adminUser._id },
    { customerId: cloudScaleClient._id, platform: "GoogleAds", accountType: "GoogleAdsAccount", platformAccountId: "act_google_cloudscale_4412", platformAccountName: "CloudScale Enterprise Google Ads", status: "Connected", accessToken: "ya29_simulated_token_cloudscale_google", connectedAt: new Date(now.getTime() - 86400000 * 20), connectedBy: adminUser._id },
  ];
...
module.exports = seedProductionFinal;
